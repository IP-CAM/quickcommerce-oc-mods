...
abstract class ResourceService
{
     ...
     * @param null $id
     */
    public function get($req, $res, $id = null, $attribute = null)
    {
		if ($id === null) {
            ...
            if (count($params) > 0) {
                ...
                foreach ($params as $key => $value) {
                    if ($i == 0) {
                        ...
                        $criteria->orWhere(Criteria::expr()->eq($key, $value));
                    }
                ...
                $data = $this->getResource()->find($criteria);
            } else {
            ...
            if ($attribute == null) {
                ...
                $entity = $this->getResource()->getEntity($id);
                $data = $entity[$attribute]; // Serialized array only

                if (is_array($data) && count($data) > 0) {
                    for ($i = 0; $i < count($data); $i++) {
                        if (array_key_exists('description', $data[$i])) { // Munge in the description for clients that need a normalized description
                            $data[$i] = array_merge($data[$i], $data[$i]['description'][0]); // TODO: This assumes English only
                        }
                    }
                }
            }
